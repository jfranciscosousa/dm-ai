import { generateTavern } from "@/actions/ai";
...
const cachedGenerateTavern = cache(generateTavern);

async function InnerTavern({ prompt }: Props) {
  const tavern = await cachedGenerateTavern(prompt);
